...
$arr = $msl->getarrayByField("SELECT subject FROM `reg_ege_minscores` a LEFT JOIN `catalogs` b ON a.specialty=b.specialty WHERE b.id = '".$r['catalog']."' LIMIT 0, 10", 'subject');
...
foreach ($arr as $a) {
    ...
    // *** TEMP ***
    if ($a == 8) $a = 4; // меняем информатику на физику - временно (ЮВ обещал доделать билеты)
    // *** TEMP ***
